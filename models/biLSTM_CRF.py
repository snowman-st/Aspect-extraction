...
class BiLSTMCRF(nn.Module):
    ...
    def __init__(self,tag_size, vocab_size,embedding_size,hidden_size,num_layers=1,weights=None):
        """
        Args:
            tag_size : int ,所有标签数目，不包含stop和start
            vocab_size : int,词典大小
            embedding_size: int, rnn输入维度
            hidden_size: int, rnn单元数
            num_layers: int, 层数
            weights: np.ndarray ,外部词向量
        """
        ...
        self.embedded = nn.Embedding(self.vocab_size+1,self.embedding_size)
        if weights is not None:
            self.embedded.weight.data.copy_(torch.from_numpy(weights))
        ...
        self.lstm = nn.LSTM(self.embedding_size,
                            ...
                            bidirectional=True,
                            batch_first=True)
        self.out = nn.Linear(self.hidden_size*2 , tag_size+2)
        self.crf = CRF(tag_size)
    def forward(self, inputs,lens,tags):
        """
        Args:
        ...
        """
        feats = self._forward_lstm(inputs,lens)
        mask = sequence_mask(lens,inputs.size(1))
        return self.crf.neg_log_likelihood_loss(feats,mask,tags).mean()

    def _forward_lstm(self,inputs,lens):
        embed = self.embedded(inputs)
        # embed.shape = (batch_size, seq_len,embedding_size)

        feats,_ = self.lstm(embed)
        # feats.shape = (batch_size, seq_len,hidden_size*2)
        # feats , _ = self._run_rnn_packed(self.lstm,embed,lens)


        out = self.out(feats)
        # out.shape = (batch_size, seq_len,tag_size)
        return torch.sigmoid(out)

    def predict(self, inputs, actual_lens, mask=None):
        feats = self._forward_lstm(inputs,actual_lens)
        batch_size = feats.size(0)
        mask = sequence_mask(actual_lens,inputs.size(1))
        tags_list = []
